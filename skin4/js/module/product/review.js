            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.write_auth == true) {
                        aHtml.push('<a href="#none" onclick="EC_PRODUCT_FRONT_BOARD_REVIEW.modify(' + data.no + ', ' + iProductNo + ');" class="btnNormal">Edit</a>');
                    }
                    aHtml.push('</span></p>');
                    aHtml.push('<p class="ec-base-button"><span class="gRight">');
                    aHtml.push('<a href="#none" class="btnNormalFix sizeS ' + data.read['report_open_btn'] +'" onclick="'+ data.read['report_open_layer_action'] +'">Report</a> ');
                    aHtml.push('<a href="#none" class="btnNormalFix sizeS ' + data.read['block_request_btn'] +'" onclick="'+ data.read['block_action'] +'">Block</a>');
                    aHtml.push('<a href="#none" class="btnNormalFix sizeS ' + data.read['unblock_request_btn'] +'" onclick="'+ data.read['unblock_action'] +'">Unblock</a>');
                    aHtml.push('</span</p>');
                    aHtml.push('</div>');
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('<p class="captcha '+data.comment_write['use_captcha']+'">'+data.comment_write['captcha_image']+data.comment_write['captcha_refresh']+data.comment_write['captcha']+'<img src="//img.echosting.cafe24.com/skin/base_en_US/product/ico_information.gif" alt="" /> Enter without any spaces. (Case sensitive)</p>');
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                    ...
                    if (data.comment_reply != undefined) {
                        ...
                        aHtml.push('<p class="captcha '+data.comment_reply['use_captcha']+'">'+data.comment_reply['captcha_image']+data.comment_reply['captcha_refresh']+data.comment_reply['captcha']+'<img src="//img.echosting.cafe24.com/skin/base_en_US/product/ico_information.gif" alt="" /> Enter without any spaces. (Case sensitive)</p>');
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
