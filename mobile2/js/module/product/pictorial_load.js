...
var globalPictorialSetUi = function(module, seq) {
    ...
    var $data = (typeof module.find('li').eq(seq).data('infomation') === 'string') ? JSON.parse(decodeURIComponent(module.find('li').eq(seq).data('infomation'))) : null,
    ...
    if (module.find('li').length < 2) { $pagenate.remove(); }

    if (module.find('li').length < 1) {
        $infomation.remove();
    } else {
        var product_price_ref_display = $data.product_price_ref_display === true ? '' : 'displaynone';

        // set detail view event
        $name.add($price).add($detail).on('click', function(){ pictorialDetailview($data.param); });
        ...
        var $effect = {
            ...
            setObject : function(){
                try {
                    ...
                    // total element
                    this.total = $(sActiveProduct).find('ul').find('li').length;
            ...
            setEvent : function() {
                this.$event = {
                     ...
                     */
                    action : function(){
                        $effect.$next.on('click', function(e){ e.stopPropagation(); $effect.next(); });
                        $effect.$prev.on('click', function(){ $effect.prev(); });
                        $effect.$module.find('li').off().on('click', function(){ globalPictorlControl($effect.$module); });
                    },
