...
var $S = {
    ...
    generateSingle : function() {
        ...
        // make li > ul > li
        for (var i=0; i<this.$productList.length; i++) { this.makeCircle(i); }
    ...
    makePagenate : function() {
        ...
        aBtn.push( '<button type="button" class="prev" onclick="$' + this.sModuleSwipe + '_slider_' + this.iActive + '.prev();return false;"><span>Previous List</span></button>' );
        aBtn.push( '<span id="' + this.sModule + '-swipe-button-' + this.iActive + '">' +  this.aCircle.join('') +'</span>' );
        aBtn.push( '<button type="button" class="next" onclick="$' + this.sModuleSwipe + '_slider_' + this.iActive + '.next();return false;"><span>Next List</span></button>' );
        aBtn.push( '</div>' );
